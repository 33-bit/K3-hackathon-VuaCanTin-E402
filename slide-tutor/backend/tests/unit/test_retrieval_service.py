...
class _LLM:
    def __init__(self, *, rerank_ids: list[UUID] | None = None) -> None:
    ...
    async def understand_query(self, **_: object) -> QueryUnderstanding:
        ...
        return QueryUnderstanding(rewritten_query="qdrant retrieval")

    async def embed_texts(self, texts: list[str]) -> list[list[float]]:
        self.embed_calls += 1
        assert texts == ["qdrant retrieval"]
        return [[0.0] * 1536]
...
async def test_selected_block_and_current_slide_survive_rerank_and_top_six(
    ...
    llm = _LLM(rerank_ids=[chunk.id for chunk in reversed(optional_chunks)])
